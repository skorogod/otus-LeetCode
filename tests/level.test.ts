...
import { levels } from "../src/api/controllers/levelController";
...
describe("test levels routes", () => {
  ...
  test("It should response body of request, after POST", async () => {
    const response = await request(app).post("/levels").send(level);
    expect(response.statusCode).toBe(200);
    expect(response.body).toEqual(level);
  });

  test("It should response body of request, after PATCH", async () => {
    const response = await request(app).patch("/levels/0").send(level);
    expect(response.statusCode).toBe(200);
    expect(response.body).toEqual(level);
  });

  test("It should send 404 status, after PATCH", async () => {
    const response = await request(app).patch("/levels/3").send(level);
    expect(response.statusCode).toBe(404);
  });

  test("It should response body of request, after PUT", async () => {
    const response = await request(app).put("/levels/0").send(level);
    expect(response.statusCode).toBe(200);
    expect(response.body).toEqual(level);
  });

  test("It should send 404 status, after PUT", async () => {
    const response = await request(app).put("/levels/3").send(level);
    expect(response.statusCode).toBe(404);
  });
