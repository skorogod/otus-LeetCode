...
import { tasks } from "../src/api/controllers/taskController";
...
describe("test tasks routes", () => {
  ...
  test("It should response body of request, after PATCH", async () => {
    const response = await request(app).patch("/tasks/0").send(task);
    expect(response.statusCode).toBe(200);
    expect(response.body).toEqual(task);
  });

  test("It should send 404 status, after PATCH", async () => {
    const response = await request(app).patch("/tasks/3").send(task);
    expect(response.statusCode).toBe(404);
  });

  test("It should response body of request, after PUT", async () => {
    const response = await request(app).put("/tasks/0").send(task);
    expect(response.statusCode).toBe(200);
    expect(response.body).toEqual(task);
  });

  test("It should send 404 status, after PUT", async () => {
    const response = await request(app).put("/tasks/3").send(task);
    expect(response.statusCode).toBe(404);
  });
